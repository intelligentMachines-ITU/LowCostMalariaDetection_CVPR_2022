...
if __name__ == '__main__':
    import glob
    path_model_prediction =   # Enter the path of the annotations folder
    textpath = path_model_prediction
    xmls_files = path_model_prediction + '/xmls/'
    draw_xmls_files = os.listdir(xmls_files)
    # draw_xmls_files = [os.path.basename(x) for x in glob.glob(xmls_files + '/*.xml')]

    arg1 = path_model_prediction + '/test.txt'
    m = 0
   
    textpath = path_model_prediction

    arg3 = path_model_prediction + '/test.json'

   
    convert(arg1, xmls_files, arg3)
